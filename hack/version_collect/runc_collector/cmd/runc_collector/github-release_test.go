...
func TestReadLibseccompVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotVer, err := ReadLibseccompVersion(tt.args.runcDownloadUrl)
			if (err != nil) != tt.wantErr {
				t.Errorf("ReadLibseccompVersion() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotVer, tt.wantVer) {
				t.Errorf("ReadLibseccompVersion() gotVer = %v, want %v", gotVer, tt.wantVer)
			}
		})
	}
}
